---
title: "BIFX552-24 Learning R on iris"
author: "Jason Ciemielewski" 
date: "08/20/2024"
format: 
  html:
    code-fold: true
...
This assignment uses material directly taken from rpubs.com with some modifications for learning purposes.
...
arr.virg <- newCol %>% filter(species == "virginica") %>%
  arrange(sepal.width)
arr.virg[30:35,] 
# will show us rows 30 through 35 and all columns
```
...
looking to plot specific variables, we can use `plot(x,y)` where `x` and `y` are the variables we're interested in. `hist()` is another useful function
...
in flowers with greater sepal widths tend to have shorter sepal lengths.
