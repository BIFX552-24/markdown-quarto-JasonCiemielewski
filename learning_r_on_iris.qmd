...
This exercise is a replication, with some modifcations for learning purposes, of Stephen Malefetsane Moerane's 'Learning R on iris' https://rpubs.com/moeransm/intro-iris
...
```{r}
# The chain operator, or the pipline %>%
...
sl_virg <- newCol %>% filter(species == "virginica") %>%
  select(sepal.length) 
print("Standard deviation in virginica sepal length (cm) is")
sd(sl_virg$sepal.length)
summarise(sl_virg, standard.deviation = sd(sl_virg$sepal.length))
```
